import socket
import webbrowser
from lib.utils import httplib2_0_15_0
...
# oauth2clientの別バージョン利用のためimport
...
class GoogleAuth(ApiAttributeMixin, object):
  ...
  def ServiceAuth(self):
    ...
    scopes = _scopes_to_string(self.settings['oauth_scope'])
  ...
  def GetFlow(self):
    ...
    self.flow = OAuth2WebServerFlow(
        ...
        _scopes_to_string(self.settings['oauth_scope']),
  ...
  def Refresh(self):
    ...
    if self.http is None:
      self.http = httplib2_0_15_0.Http(timeout=self.http_timeout)
    try:
  ...
  def Get_Http_Object(self):
    ...
    :rtype: httplib2.Http
    """
    http = httplib2_0_15_0.Http(timeout=self.http_timeout)
